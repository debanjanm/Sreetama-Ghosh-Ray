...
The study uses a structured five-point agreement questionnaire. It is quantitative, descriptive, correlational, and cross-sectional. The findings describe the selected respondent group and relationships among reported perceptions. They do not establish that training caused later changes in work behaviour or bank performance.
...
Chapter 3 presents the research methodology. It explains the objectives, relationship hypotheses, variable definitions, research design, sample approach, data-collection procedure, questionnaire, scoring method, reliability and validity plan, ethical considerations, and limitations, and reports how the completed data collection was cleaned.

Chapter 4 presents the completed data analysis and interpretation, including respondent profile, descriptive statistics, reliability, correlation, regression results, and their practical implications.

Chapter 5 reports the conclusions, contributions, limitations, and future research directions arising from the completed analysis.
...
These are possible areas for interpretation, not conclusions in advance. A cross-sectional survey shows how selected employees report one recent programme. It does not establish that a particular LMS feature, trainer action, or learning sequence caused a later improvement. The literature provides a sound framework for interpreting employee feedback and considering practical improvements after the completed data are analysed.
...
The fourth gap is a **measurement gap**. A validated blended-learning environment scale is available in a student setting, while a workplace training-effectiveness scale includes organisation-level outcomes. Neither can be used unchanged for a short survey of Union Bank employees. The present study uses a literature-informed questionnaire that separates programme design, trainer competence, learner engagement, and employee-level training effectiveness. The instrument was reviewed by the academic guide and its internal consistency was assessed using the final survey data; no separate pilot was conducted before data collection.
...
The research instrument is a structured, self-administered questionnaire placed in Appendix A. It has 30 numbered questions. Questions 1–4 screen for eligibility and describe the respondent group. Questions 5–30 use a five-point agreement scale from Strongly Disagree to Strongly Agree. Each eligible respondent answered about one hybrid-training programme attended during the preceding 12 months.
...
The three hypotheses are stated as relationships. Pearson correlation is therefore the primary test for deciding whether each reported relationship is positive and statistically significant. The null hypothesis is rejected when the relevant *p*-value is below .05. Regression analysis provides supporting model-level evidence. Chapter 4 presents correlation coefficients, regression coefficients, test statistics, *p*-values, and explained variance, together with tolerance and Variance Inflation Factor values for the two-predictor model. Because predictor overlap is present, Chapter 4 does not claim that one predictor has a stronger independent relationship than the other. A statistically significant finding is described as an association in the retained sample, not proof that one construct caused another.
...
A separate pilot study was not conducted before main data collection. The questionnaire was finalised after academic-guide approval and was prepared from the documented questionnaire evidence map, the selected base papers, and supporting measurement literature.
...
The questionnaire is literature-informed and adapted to the selected Union Bank setting. It is not presented as a ready-made or fully validated bank-specific scale. Its limitations are stated in Chapter 5 alongside the final reliability results.
