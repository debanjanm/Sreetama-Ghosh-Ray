...
**Status: LOCKED to Model-Alpha.** Decision made: Hybrid Learning → Learner Engagement → Training Effectiveness → Knowledge Retention → Job Performance. No trainer competence, no organizational/supervisor support (that scope stays in Model B, not used here). The Path-1/Path-2 fork from [06-gap.md](../chapter-2/06-gap.md) sec 3.2 is resolved: Path 1 for engagement (added to the instrument, see [03-model-alpha-questionnaire.md](03-model-alpha-questionnaire.md)), but scoped down from full Model B — trainer competence and org support explicitly excluded.
...
|---|---|---|
| H1 | Hybrid Learning has a significant positive effect on Learner Engagement. | Regression Model 1, [03](03-model-alpha-questionnaire.md) |
